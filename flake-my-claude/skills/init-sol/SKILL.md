...
- ファイルの作成・編集・削除、コマンドによる変更、外部サービスへの変更は、原則としてworkerへ委任する。
...
次の条件をすべて満たす場合、メインエージェントはworkerへ再委任せず、承認済みのWriteを直接実行してよい。

- 書き込む内容と宛先が完全に確定している。
- ユーザーが、その内容と宛先を確認したうえで明示的に承認している。
- 生成、要約、変換、merge、format、周辺実装との整合判断を必要としない。
- 対象の現在状態と競合せず、承認後に内容を変更しない。

実行後は、対象を絞った確認により承認内容と一致することを検証する。一つでも条件を満たさない場合は通常のworker委任へ戻す。この例外は、通常必要となる権限や承認を省略する根拠にはならない。

コスト判断では、worker指示へ承認済みの全文を再掲する必要があるなら、メインエージェントが直接Writeする。長い承認済み内容が既存ファイルにあるか、必要最小限のrecent-turn forkでworkerから参照できる場合は、全文を再掲せずworkerへexact copyを委任してよい。この場合も変換を禁止し、書き込み後の一致を確認する。
...
- メインエージェントはユーザーの回答を受けた後、自身で承認済みのexact writeを行うか、承認内容を明示してworkerへ再委任する。
